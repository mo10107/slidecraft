...
- **Slide 1**: Title / Introduction slide — large title, subtitle, and a brief one-sentence hook
- **Slides 2 to {CONTENT_SLIDES_END}**: One slide per outline item (content slides)
- **Last Slide**: Conclusion / Thank You slide — key takeaway summary and call to action
...
2. **Slide 1 MUST be a Title/Introduction slide** (H1 title, subtitle, brief hook)
3. **Last slide MUST be a Conclusion/Thank You slide** (key takeaways and call to action)
4. Use DIFFERENT layouts for consecutive slides - never repeat
5. Expand outline content - do NOT copy verbatim
6. Include detailed image queries on most slides
7. Vary SECTION layout attribute (left/right/vertical) throughout
8. Use ONLY layout tags from AVAILABLE LAYOUTS - unlisted tags cause parsing errors`;
  }
...
2. **Slide 1 MUST be a Title/Introduction slide** (H1 title, subtitle, brief hook)
3. **Last slide MUST be a Conclusion/Thank You slide** (key takeaways and call to action)
4. **MUST USE ALL SELECTED TEMPLATES**: You have ${selectedTemplateCount} selected template(s) - each MUST appear at least once
5. **REMAINING SLIDES**: Fill the other ${totalSlides - selectedTemplateCount} slides using layouts from ADDITIONAL LAYOUTS
6. Expand outline content - do NOT copy verbatim
7. You may add \`<IMG query="..." />\` tags for images
8. Vary SECTION layout attribute (left/right/vertical) throughout
9. For per-slide assignments: use the EXACT template specified`;
  }
...
2. **Slide 1 MUST be a Title/Introduction slide** (H1 title, subtitle, brief hook)
3. **Last slide MUST be a Conclusion/Thank You slide** (key takeaways and call to action)
4. **TEMPLATE CONSTRAINT**: Use ONLY layouts from AVAILABLE LAYOUTS. Any other tag = parsing failure
5. Expand outline content - do NOT copy verbatim
6. You may add \`<IMG query="..." />\` tags - this is the ONLY modification allowed
7. Vary SECTION layout attribute (left/right/vertical) throughout
8. For per-slide assignments: use the EXACT template specified with NO structural changes`;
}
...
export async function POST(req: Request) {
  ...
  try {
    ...
    const contentSlidesEnd = outline.length + 1; // last content slide index
    ...
    const stream = await chain.stream({
      ...
      CONTENT_SLIDES_END: contentSlidesEnd,
