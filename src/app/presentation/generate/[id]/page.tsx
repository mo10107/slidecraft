...
export default function PresentationGenerateWithIdPage() {
  ...
  const canGenerateImageSlides = Boolean(session?.user);
  ...
  const { data: presentationData, isLoading: isLoadingPresentation } = useQuery(
    {
      ...
      // Don't fetch while actively generating the outline — we already have
      // (or are building) the data in Zustand.  Only fetch when idle with no
      // local outline data (e.g., direct navigation or page refresh).
      enabled:
        !isGeneratingOutline &&
        (currentPresentationId !== id || outline.length === 0),
    },
  );
  ...
  // Only show a full-page blocking spinner when we truly have no data to show:
  // the DB fetch is in-flight AND we are not generating AND we have no outline.
  if (isLoadingPresentation && !isGeneratingOutline && !hasOutline) {
    return (
      <ThemeBackground
