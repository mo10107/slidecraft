...
export function PresentationGenerationManager() {
  ...
  // Always holds the last successfully parsed outline items (survives RAF flush)
  const latestParsedOutlineRef = useRef<string[]>([]);
  ...
  const processMessages = (messages: typeof outlineMessages): void => {
    ...
    if (latestOutlineItems.length > 0) {
      ...
      latestParsedOutlineRef.current = latestOutlineItems;
    }
  };
  ...
  const {
    ...
    onFinish: () => {
      // onFinish fires before React re-renders with the final message, so the
      // Zustand outline may still be empty (setOutline is queued via RAF).
      // Fix: cancel pending RAF and flush the buffer to Zustand synchronously
      // right now, then fall back to the last ref-tracked outline.
      if (outlineRafIdRef.current !== null) {
        cancelAnimationFrame(outlineRafIdRef.current);
        outlineRafIdRef.current = null;
      }
      updateOutlineWithRAF(); // synchronous flush of outlineBufferRef → Zustand

      const {
        currentPresentationId,
        outline: zustandOutline,
        searchResults,
        currentPresentationTitle,
        imageSource,
      } = usePresentationState.getState();

      // Use Zustand state if populated; otherwise fall back to the last parsed ref
      const outline = hasGeneratedOutline(zustandOutline)
        ? zustandOutline
        : latestParsedOutlineRef.current;
      ...
      // Ensure Zustand has the outline even if RAF hadn't fired yet
      if (!hasGeneratedOutline(zustandOutline) && outline.length > 0) {
        setOutline(outline);
      }
          ...
          void (async () => {
            try {
              ...
              if (usesStockSearch) {
                ...
                // Automatic mode: if stock search failed, fall back to AI generation
